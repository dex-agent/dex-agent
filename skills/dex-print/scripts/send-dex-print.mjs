...
function resolveConfig(args) {
  ...
  const chatId =
    args.chatId ||
    process.env.DEX_PRINT_CHAT_ID ||
    process.env.TELEGRAM_CHAT_ID ||
    firstId(process.env.PROACTIVE_USER_IDS) ||
    firstId(process.env.ALLOWED_USER_IDS);

  if (!token) {
    throw new Error("BOT_TOKEN ausente. Configure BOT_TOKEN/TELEGRAM_BOT_TOKEN ou carregue o .env do Dex Agent.");
  }
  if (!chatId) {
    throw new Error("chat_id ausente. Passe --chat-id ou configure DEX_PRINT_CHAT_ID/TELEGRAM_CHAT_ID/PROACTIVE_USER_IDS/ALLOWED_USER_IDS.");
  }
...
function validatePaths(paths) {
  const resolved = paths.map((filePath) => path.resolve(process.cwd(), filePath));
  if (!resolved.length) {
...
async function sendTelegramFile({ token, chatId, filePath, caption, mode }) {
  ...
  form.set(field, await fs.openAsBlob(filePath, { type: mimeFromFile(filePath) }), path.basename(filePath));
...
async function sendWithFallback(config, filePath, caption, requestedMode) {
  ...
  try {
    const result = await sendTelegramFile({ ...config, filePath, caption, mode: preferred });
    return { ok: true, file: filePath, mode: preferred, message_id: result.message_id, chat_id: result.chat?.id };
  } catch (error) {
    if (!allowFallback || preferred === "document") {
      return { ok: false, file: filePath, mode: preferred, error: error.message };
    }
    try {
      const result = await sendTelegramFile({ ...config, filePath, caption, mode: "document" });
      return { ok: true, file: filePath, mode: "document", message_id: result.message_id, chat_id: result.chat?.id, fallback_from: "photo" };
    } catch (fallbackError) {
      return { ok: false, file: filePath, mode: "document", error: fallbackError.message, fallback_from: "photo", original_error: error.message };
    }
  }
}
...
function formatText(results) {
  ...
  for (const item of results) {
    if (item.ok) {
      lines.push(`- ${path.basename(item.file)} -> ${item.mode} message_id=${item.message_id}`);
    } else {
...
if (args.dryRun) {
  const output = {
    ...
    files: files.map((filePath) => ({ file: filePath, bytes: fs.statSync(filePath).size }))
  };
  if (args.json) {
    console.log(JSON.stringify(output, null, 2));
  } else {
    console.log(formatText(output.files.map((item) => ({ ok: true, file: item.file, mode: args.mode, message_id: "dry-run" }))));
  }
  process.exit(0);
}
...
for (let index = 0; index < files.length; index += 1) {
  ...
  const caption = `${prefix}${args.caption || path.basename(filePath)}`.slice(0, 1024);
