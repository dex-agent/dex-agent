...
test("restart bootstrap waits for the parent process before spawning the next bot", () => {
  ...
  assert.match(script, /restartScriptPath = path\.join\(repoCwd, 'scripts', 'restart-dex-agent-hidden\.vbs'\)/);
  assert.match(
    script,
    /spawn\(wscriptPath, \[restartScriptPath\]/
  );
  assert.match(script, /cwd: repoCwd/);
  assert.match(script, /windowsHide: true/);
  assert.match(script, /logLine\(`spawned restart via \$\{restartScriptPath\}`\)/);
});
